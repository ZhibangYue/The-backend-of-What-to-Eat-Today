...
# 文件存储位置
STATIC_FILE_LOCATION = "/home/site/wwwroot"
...
async def edit_dish(dish_message: DishMessage, dish_id: str = Body(), db: Session = Depends(get_db),
    ...
    if dish.photos:
        if dish.photos != dish_message.photos:
            if os.path.exists(STATIC_FILE_LOCATION + dish.photos):
                os.remove(STATIC_FILE_LOCATION + dish.photos)
    if dish.spare_photos:
        if dish.spare_photos != dish_message.spare_photos:
            if os.path.exists(STATIC_FILE_LOCATION + dish.spare_photos):
                os.remove(STATIC_FILE_LOCATION + dish.spare_photos)
...
async def add_photo(photo: UploadFile, current_manager: ManagerMessage = Depends(get_current_manager)
    ...
    async with aiofiles.open(STATIC_FILE_LOCATION + file_path, "wb") as f:
